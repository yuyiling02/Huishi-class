...
const BioDigitalViewer: React.FC<BioDigitalViewerProps> = ({ src, onFallback }) => {
  ...
  useEffect(() => {
    return () => {
      const frame = document.getElementById('biodigital-iframe') as HTMLIFrameElement | null;
      if (frame) {
    ...
    <div className="relative h-full w-full bg-white">
      ...
      <div className="absolute left-6 top-6 z-20 flex gap-2">
        <button
          type="button"
          onClick={onFallback}
          className="flex items-center gap-2 rounded-xl border border-line/60 bg-white/85 px-4 py-2 text-[10px] font-black text-emerald-600 shadow-sm backdrop-blur-md transition hover:bg-white"
        ...
        <button
          ...
          className="flex items-center gap-2 rounded-xl border border-line/60 bg-white/85 px-4 py-2 text-[10px] font-black text-gray-600 shadow-sm backdrop-blur-md transition hover:bg-white"
      ...
      {!isLoaded && (
        <div className="absolute inset-0 z-10 flex items-center justify-center bg-white/80 px-6 backdrop-blur-md">
          <div className="max-w-md rounded-[24px] border border-line bg-white/90 p-6 text-center shadow-xl">
